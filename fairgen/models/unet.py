    ...
    def forward(
        ...
        if controlnet_cond is not None:
            if controlnet_interpolator is not None:
                ...
                sample =sample+self.controlnet(controlnet_cond, emb) # 

        if controlnet_cond_guidance is not None:
            _nsize = sample.shape[0]//2
            if controlnet_interpolator is not None:
                sample[1:2]=controlnet_interpolator(sample[1:2],self.controlnet(controlnet_cond_guidance,emb[1]))
            else:
                sample[1:2]=sample[1:2]+self.controlnet(controlnet_cond_guidance,emb[1])
                # sample[1:2]=(sample[1:2].permute(2, 0, 1, 3, 4)+self.controlnet(controlnet_cond_guidance, emb[1]).unsqueeze(0).expand(16, -1, -1, -1, -1)).permute(1, 2, 0, 3, 4)
            # sample_list.append(sample[1:2])
        # up
